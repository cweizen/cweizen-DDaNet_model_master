...
class FeatureExtractor():
    ...
    def __call__(self, cx, dx):
        ...
        cx.register_hook(self.save_gradient)
        outputs += [cx]
...
def test(model,topk):
    ...
    for ordata, data, target in test_loader:
        ...
        for inum in range(len(pred)):
            ...
            if int(target[inum])==4 or int(target[inum])==10 or int(target[inum])==11 or int(target[inum])==15:# or int(target[inum])==4 or int(target[inum])==17:# r 17 e 4 p 15 n 13 w 22
                # original data
                ordata_RGB, ordata_D = ordata[0], ordata[1]
                Odata = ordata_RGB[inum].numpy()  
                RGBdata = np.zeros((Odata.shape[0],Odata.shape[1],3),dtype = np.uint8)
                
                for ix in range(Odata.shape[0]):
                    for iy in range(Odata.shape[1]):
                        for iz in range(3):
                            RGBdata[ix,iy,iz]=int(Odata[ix,iy,iz])
		    ...
		    # print(savestream)
                    ...
                    # save original image
                    if not os.path.exists(savestream+"/original_RGB/"):
                        os.makedirs(savestream+"/original_RGB/")
                    plt.imshow(RGBdata)
                    plt.savefig(savestream+"/original_RGB/"+str(256*batchindex+inum)+".png")

...
class FeatureExtractor():
    ...
    def __call__(self, cx, dx):
        ...
        ## get this layer's featuremap
        dx.register_hook(self.save_gradient)
        outputs += [dx]
...
class GradCam:
	...
	def __call__(self, input_color, input_depth, index = None):
		...
		self.model.featurec1.zero_grad()
		self.model.featurec2.zero_grad()
		self.model.featurec3.zero_grad()
		self.model.featurec4.zero_grad()
		self.model.featured1.zero_grad()
		self.model.featured2.zero_grad()
		self.model.featured3.zero_grad()
		self.model.featureb1.zero_grad()
		self.model.featureb2.zero_grad()
		self.model.featureb3.zero_grad()
		...
		#weights = np.mean(grads_val, axis = (2, 3))[0, :].astype(np.float64)
		cam = np.zeros(target.shape[1 : ], dtype = np.float64)
...
def test(model,topk):
    ...
    for ordata, data, target in test_loader:
        ...
        for inum in range(len(pred)):
            ...
            if int(target[inum])==4 or int(target[inum])==10 or int(target[inum])==11 or int(target[inum])==15 or int(target[inum])==16 or int(target[inum])==18 or int(target[inum])==19 or int(target[inum])==23:# or int(target[inum])==4 or int(target[inum])==17:# r 17 e 4 p 15 n 13 w 22
                # original data
		    ...
		    # print(savestream)
                    ...
                    # save original image
                    if not os.path.exists(savestream+"/original_depth/"):
                        os.makedirs(savestream+"/original_depth/")
                    plt.imshow(Ddata,cmap='gray')
                    plt.title("("+arrayindex[int(target[inum])]+","+arrayindex[int(pred[inum])]+")")
                    plt.savefig(savestream+"/original_depth/"+str(256*batchindex+inum)+".png")
